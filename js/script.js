...
// ========================================================
// --- 6. КИНО БРОНДОО БӨЛҮМҮ (ТОЛУК ЖАНА ИШЕНДҮҮ) ---
// ========================================================

let selectedSessionPrice = 0; 

// 1. МОДАЛДЫ АЧУУ ЖАНА СЕАНСТАРДЫ ЖҮКТӨӨ
window.openBooking = async (movieId, title) => {
    if (!auth.currentUser) {
        showToast("Алгач профилиңизге кириңиз!", "info");
    ...
    // Сеанстарды реалдуу убакытта алуу (Snapshot)
    onSnapshot(doc(db, "movies", movieId), async (docSnap) => {
        ...
        for (const s of sessions) {
            ...
            if (freeSeats <= 0) {
                ...
                chip.innerHTML = `<span>${s.time}</span><br><small style="font-size:9px; color:#ff4757;">Толду</small>`;
...
window.goToStep2 = () => {
    ...
    const mbankNumber = "0700 123 456"; 
    
    document.getElementById('payment-instruction').innerHTML = `
        <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 15px; border: 1px dashed #a855f7; text-align: center;">
            <p style="color: #fff; margin: 0; font-size: 13px;">Жалпы сумма:</p>
            <h2 style="color: #00ffcc; margin: 5px 0; font-size: 26px; font-weight: 900;">${totalAmount} сом</h2>
            
            <div style="background: rgba(168, 85, 247, 0.1); padding: 10px; border-radius: 12px; margin-bottom: 10px;">
                <p style="color: #ec4899; margin: 0; font-size: 11px; font-weight: bold;">МБАНК РЕКВИЗИТ:</p>
                <p style="color: #fff; margin: 5px 0 0 0; font-size: 18px; font-weight: 800;">${mbankNumber}</p>
            </div>
            
            <p style="color: #00ffcc; font-size: 11px; font-weight: 500;">
                Сумманы төлөп, чекти ылдый жака тиркеп жөнөтүңүз!
            </p>
        </div>
    `;
...
// 3. ТӨЛӨМДҮ ЖӨНӨТҮҮ (ImgBB + Firebase)
window.handlePaymentSubmit = async () => {
    ...
    // HTML-де баскычты туура табуу
    const btn = document.querySelector('#step-2 .btn-main');
    ...
    try {
        ...
        if (!imgJson.success) throw new Error("Сүрөт жүктөлгөн жок");

        const checkUrl = imgJson.data.url;
                // 2. Firestore'го жазуу
        const adultCount = Number(document.getElementById('ticket-count').value) || 0;
        const childCount = Number(document.getElementById('children-count').value) || 0;
        ...
        await addDoc(collection(db, "user_tickets"), {
            ...
            // Бул жерге count талаасын коштук, ошондо билетте саны көрүнөт
        ...
        showToast("Өтүнмө ийгиликтүү жөнөтүлдү!", "success");
...
window.clearBookingForm = () => {
    ...
    if(document.getElementById('check-file-input')) document.getElementById('check-file-input').value = "";
    if(document.getElementById('file-status')) document.getElementById('file-status').innerText = "Чекти жүктөө";
...
// 2. ФОРМАНЫ ТАЗАЛОО
window.clearAtknForm = () => {
    const ids = ['atkn-user-name', 'atkn-user-phone', 'atkn-adults', 'atkn-children'];
    ids.forEach(id => {
        const el = document.getElementById(id);
...
window.loadAtknMenu = async () => {
    ...
    try {
        ...
        querySnapshot.forEach((docSnap) => {
            ...
            const itemHTML = `
                <div class="atkn-menu-row" style="display: flex; align-items: center; justify-content: space-between; padding: 10px; border-bottom: 1px solid rgba(255,255,255,0.05);">
                    <div style="flex: 1;">
                        <div style="color: white; font-size: 13px;">${item.name}</div>
                        <div style="color: #00ffcc; font-size: 12px; font-weight: bold;">${item.price} с</div>
                    </div>
                    <div style="display: flex; align-items: center; gap: 10px; background: rgba(255,255,255,0.08); padding: 5px; border-radius: 8px;">
                        <button type="button" onclick="window.updateAtknQty('${itemId}', -1, '${item.name}', ${item.price})" style="background:none; border:none; color:#ff4757; font-size:18px; cursor:pointer; width:25px;">−</button>
                        <span id="qty-${itemId}" style="color: white; font-weight: bold; min-width: 15px; text-align: center;">0</span>
                        <button type="button" onclick="window.updateAtknQty('${itemId}', 1, '${item.name}', ${item.price})" style="background:none; border:none; color:#00ffcc; font-size:18px; cursor:pointer; width:25px;">+</button>
                    </div>
                </div>`;
            
            if (item.type === "food") foodList.innerHTML += itemHTML;
            else drinkList.innerHTML += itemHTML;
        });
    } catch (err) { console.error("Меню жүктөө катасы:", err); }
};
...
window.updateAtknQty = (id, delta, name, price) => {
    ...
    if (qty > 0) {
        selectedAtknFood[id] = { name: name, count: qty, price: Number(price) };
    } else {
        delete selectedAtknFood[id];
    }
};

// 5. ТӨЛӨМГӨ ӨТҮҮ (STEP 4)
window.goToStep4 = () => {
    ...
    if (adults < 1 || !selectedTime) {
        return showToast("Адам санын жана убакытты тандаңыз!", "info");
    }

    // Баа эсептөө логикасы
    ...
    if (payInfo) {
        ...
        tag.innerHTML = `
            <div style="text-align:center; margin-bottom:15px; background:rgba(0,255,204,0.1); padding:15px; border-radius:15px; border: 1px solid #00ffcc;">
                <h2 style="color:#00ffcc; margin:0;">${total} сом</h2>
                <div style="font-size:11px; color:white; opacity:0.7;">Зал: ${roomPrice}с | Меню: ${foodSum}с | Балдар: ${children * 200}с</div>
            </div>`;
        payInfo.prepend(tag);
    }
...
window.handleAtknSubmit = async (e) => {
    ...
    try {
        ...
        if (!img.success) throw new Error("Сүрөт жүктөлгөн жок");
...
await addDoc(collection(db, "user_tickets"), {
    ...
    // БУЛ ЖЕРДИ ӨЗГӨРТТҮК:
    adults: adults,       // Чоңдордун саны өзүнчө
    children: children,   // Балдардын саны өзүнчө
    count: adults, // Жалпы саны (керек болсо)
    ...
    createdAt: serverTimestamp()
        ...
        console.error(err);
        showToast("Ката кетти! Сүрөттү текшериңиз.", "error"); 
    } finally { 
        btn.disabled = false; 
        btn.innerText = "Ырастоо"; 
    }
...
window.updateAtknFileName = () => {
    ...
    if (input && input.files[0]) text.innerText = input.files[0].name;
};
...
window.toggleAccordion = (id) => {
    ...
    if (isHidden) {
        ...
        const arrow = content.parentElement.querySelector('.arrow-icon');
    ...
    onSnapshot(collection(db, "user_tickets"), (snap) => {
        ...
        snap.forEach(docSnap => {
            ...
            if (t.userId === uid) {
                ...
                const statusColor = isCheckedIn ? "#94a3b8" : (isApproved ? "#2ecc71" : "#f39c12");
                const statusText = isCheckedIn ? "КОЛДОНУЛДУ" : (isApproved ? "ДАЯР" : "КҮТҮҮДӨ");
                ...
                ticketCard.style.cssText = `
                    ...
                    filter: ${isCheckedIn ? 'grayscale(1) opacity(0.6)' : 'drop-shadow(0 15px 30px rgba(0,0,0,0.2))'};
                `;
    ...
    <svg viewBox="0 0 600 260" xmlns="http://www.w3.org/2000/svg" style="width:100%; height:auto; display:block;">
        <defs>
            <mask id="m-${id}">
                ...
                <g fill="black">
                    ${[50, 85, 120, 155, 190, 225].map(y => `<circle cx="0" cy="${y}" r="6"/><circle cx="600" cy="${y}" r="6"/>`).join('')}
        ...
        <foreignObject x="0" y="0" width="415" height="260">
            <div xmlns="http://www.w3.org/1999/xhtml" style="height:100%; padding:25px 30px; color:white; display:flex; flex-direction:column; justify-content:center; box-sizing:border-box; font-family:sans-serif; position:relative;">
                ${isCheckedIn ? `<div style="position:absolute; top:50%; left:50%; transform:translate(-50%, -50%) rotate(-15deg); border:4px solid #ff4757; padding:10px 20px; font-size:35px; font-weight:900; color:#ff4757; z-index:10; border-radius:10px; text-transform:uppercase; background:rgba(0,0,0,0.1); white-space:nowrap;">ТЕКШЕРИЛДИ</div>` : ''}
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:10px;">
                    <div style="width:12px; height:12px; border-radius:50%; background:${statusColor}"></div>
                    <span style="font-size:16px; font-weight:800; color:${statusColor}; letter-spacing:1.5px; text-transform:uppercase;">${statusText}</span>
                </div>
                <h2 style="margin:0 0 20px 0; font-size:36px; font-weight:900; line-height:1; letter-spacing:0.5px; overflow:hidden; white-space:nowrap; text-overflow:ellipsis; ${isCheckedIn ? 'text-decoration:line-through; opacity:0.5;' : ''}">
                    ${t.movieTitle.toUpperCase()}
                </h2>
                <div style="display:flex; justify-content: space-between; gap:15px;">
                    <div style="flex: 2;"><p style="margin:0; font-size:15px; color:#94a3b8; font-weight:700;">ЭЭСИ</p><p style="margin:6px 0; font-size:20px; font-weight:700;">${t.userName}</p></div>
                    <div style="flex: 1.4;"><p style="margin:0; font-size:15px; color:#94a3b8; font-weight:700;">УБАКТЫСЫ</p><p style="margin:6px 0; font-size:20px; font-weight:800; color:#e6d8b5;">${t.sessionTime}</p></div>
                    <div style="flex: 1;"><p style="margin:0; font-size:15px; color:#94a3b8; font-weight:700;">САНЫ</p><p style="margin:6px 0; font-size:20px; font-weight:700;">${t.count} к.</p></div>
                </div>
            </div>
        </foreignObject>
        ...
        <foreignObject x="415" y="0" width="185" height="260">
            <div xmlns="http://www.w3.org/1999/xhtml" style="height:100%; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:15px; box-sizing:border-box;">
                <div id="qr-${id}" style="width:125px; height:125px; background:white; border-radius:10px; box-shadow: 0 4px 10px rgba(0,0,0,0.1); display:flex; align-items:center; justify-content:center; overflow:hidden;">
                    ${isCheckedIn ? 
                        `<div style="text-align:center; color:#ff4757;">
                            <span class="material-icons-round" style="font-size:50px; display:block;">task_alt</span>
                            <span style="font-size:11px; font-weight:900;">ОКУЛДУ</span>
                        </div>` : 
                        (!isApproved ? `<div style="color:#555; font-size:13px; text-align:center; font-weight:bold; padding:10px;">КҮТҮҮ...</div>` : '')
                    }
                </div>
...
`;
                ...
                // QR кодду түзүү (Билет checked-in эмес жана approved болсо)
                if (isApproved && !isCheckedIn) {
                    setTimeout(() => {
                        const qrElem = document.getElementById(`qr-${id}`);
                        if (qrElem) {
                            // Ичинде текст жок экенин текшерип (мисалы "КҮТҮҮ"), анан тазалап салабыз
                            qrElem.innerHTML = "";
                            new QRCode(qrElem, {
                                text: id,
                                width: 110,
                    ...
                    }, 200); // Таймерди бир аз көбөйттүк DOM толук даяр болушу үчүн
                }
            }
        });
    });
