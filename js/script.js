...
window.goToStep2 = () => {
    ...
    // ЖАҢЫ ЭСЕПТӨӨ ЛОГИКАСЫ:
    // Чоңдор = Сеанстын баасы * саны
    // Балдар (7-9 ж) = 100 сом * саны
    ...
    document.getElementById('payment-instruction').innerHTML = `
        <div style="background: rgba(255,255,255,0.05); padding: 20px; border-radius: 20px; border: 1px dashed rgba(168, 85, 247, 0.4); text-align: center;">
            ...
            <h2 style="background: linear-gradient(135deg, #a855f7 0%, #ec4899 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin: 5px 0 15px 0; font-size: 28px; font-weight: 900;">${totalAmount} сом</h2>
            
            <div style="background: rgba(168, 85, 247, 0.1); padding: 15px; border-radius: 15px; border: 1px solid rgba(168, 85, 247, 0.2);">
                <p style="color: #ec4899; margin: 0; font-size: 11px; font-weight: bold; text-transform: uppercase; letter-spacing: 1px;">МБанк реквизит:</p>
                <p style="color: #fff; margin: 5px 0 0 0; font-size: 22px; font-weight: 800; letter-spacing: 2px;">${mbankNumber}</p>
            </div>
            
            <p style="color: rgba(255,255,255,0.4); font-size: 11px; margin-top: 15px; line-height: 1.4;">
                * Которуу жасап, чекти төмөнгө жүктөңүз.
            </p>
        </div>
    `;
...
window.handlePaymentSubmit = async () => {
    ...
    try {
        ...
        document.getElementById('check-file-input').value = "";
        document.getElementById('file-status').innerText = "Файлды тандоо";
...
// ========================================================
// --- 7. АНТИ-КИНО БӨЛҮМҮ (HTML ГЕ ШАЙКЕШ КЕЛТИРИЛДИ) ---
// ========================================================
...
// 2. АККОРДЕОН (Сиздин HTML'деги toggleAccordion аты менен)
...
// 3. МЕНЮНУ ЖҮКТӨӨ (+- БАСКЫЧТАР МЕНЕН)
...
window.goToStep4 = () => {
    ...
    if (payInfo) {
        ...
        tag.innerHTML = `
            <div style="text-align:center; margin-bottom:15px; background:rgba(0,255,204,0.1); padding:15px; border-radius:15px;">
                <h2 style="color:#00ffcc; margin:0;">${total} сом</h2>
                <div style="font-size:12px; opacity:0.6;">Зал: ${roomPrice}с | Меню: ${foodSum}с</div>
            </div>`;
        payInfo.prepend(tag);
    }
...
window.handleAtknSubmit = async (e) => {
    ...
    try {
        ...
        const foodArray = Object.values(selectedAtknFood).map(f => ({ name: f.name, count: f.count }));
        ...
        showToast("Ийгиликтүү жөнөтүлдү!");
        window.closeAtknModal();
