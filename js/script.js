...
import { 
    ...
    doc, serverTimestamp, deleteDoc, getDocs, query 
...
let currentMovieMaxSeats = 34; // Демейки маани
...
window.openBooking = async (movieId, title) => {
    ...
    // Кадамдарды баштапкы абалга келтирүү
    window.goToStep1();

    onSnapshot(doc(db, "movies", movieId), async (docSnap) => {
        if (!docSnap.exists()) return chipsCont.innerHTML = "Маалымат жок";
        
        const movieData = docSnap.data();
        const sessions = movieData.sessions || [];
        currentMovieMaxSeats = movieData.maxSeats || 34; // Базадан орун санын алуу
        chipsCont.innerHTML = "";
        ...
        // Ар бир сеанс үчүн бош орундарды текшерүү
        for (const s of sessions) {
            ...
            const freeSeats = currentMovieMaxSeats - occupied;
            const chip = document.createElement('div');
            chip.className = "time-chip";
            
            if (freeSeats <= 0) {
                // БИЛЕТ ТҮГӨНГӨНДӨ: Чийилип жана басылбай калат
                chip.classList.add('sold-out');
                chip.innerHTML = `<span>${s.time}</span><br><small style="font-size:9px; color:#ff4757; text-decoration: none;">Бош орун жок</small>`;
                chip.style.textDecoration = "line-through";
                chip.style.opacity = "0.6";
                chip.style.pointerEvents = "none";
            } else {
                chip.innerHTML = `<span>${s.time}</span><br><small style="font-size:9px; color:#00ffcc;">${freeSeats} орун бар</small>`;
                chip.onclick = () => {
...
window.closeBookingModal = () => {
    ...
    clearBookingForm(); // Модал жабылганда форманы тазалоо
};
...
// ФОРМАНЫ ТАЗАЛОО ФУНКЦИЯСЫ
function clearBookingForm() {
    document.getElementById('user-name').value = "";
    document.getElementById('user-phone').value = "";
    document.getElementById('selected-session-time').value = "";
    ...
    if (fileStatus) fileStatus.innerText = "Файлды тандоо";
...
window.goToStep2 = () => {
    ...
    if (!name || !phone || !sess || (adultCount + childCount) === 0) {
        return showToast("Маалыматтарды толук толтуруңуз жана сеансты тандаңыз!", "info");
    }
    ...
    document.getElementById('payment-instruction').innerHTML = `
        <div style="background: rgba(255,255,255,0.05); padding: 20px; border-radius: 20px; border: 1px dashed rgba(168, 85, 247, 0.4); text-align: center;">
            <p style="color: rgba(255,255,255,0.6); margin: 0; font-size: 13px;">Төлөй турган сумма:</p>
            <h2 style="color: #ec4899; margin: 5px 0 15px 0; font-size: 28px; font-weight: 900;">${totalAmount} сом</h2>
            <div style="background: rgba(168, 85, 247, 0.1); padding: 15px; border-radius: 15px;">
...
window.handlePaymentSubmit = async () => {
    ...
    try {
        ...
        clearBookingForm(); // Ийгиликтүү бүткөндө форманы тазалоо
        closeBookingModal();
...
// ========================================================
// --- 7. АНТИ-КИНО БӨЛҮМҮ (ТОЛУК ЖАНА ЖАҢЫЛАНГАН) ---
// ========================================================
...
window.clearAtknForm = () => {
    ...
    // Тандалган тамактарды тазалоо
    selectedAtknFood = {};
    
    // Жалпы сумманы өчүрүү
    const totalTag = document.getElementById('atkn-total-price-tag');
    if (totalTag) totalTag.remove();

    // Биринчи сеансты тандап коюу
    const firstTime = document.querySelector('input[name="atkn-time"]');
...
window.openAtknModal = async (roomName) => {
    ...
    if (modal) {
        window.clearAtknForm(); // Ачууда баарын тазалайбыз
...
window.loadAtknMenu = async () => {
    ...
    try {
        ...
        foodList.innerHTML = ""; drinkList.innerHTML = ""; 
            ...
            const itemHTML = `
                <div class="atkn-menu-row" style="display: flex; align-items: center; justify-content: space-between; padding: 12px; border-bottom: 1px solid rgba(255,255,255,0.05);">
                    ...
                    <div style="display: flex; align-items: center; gap: 12px; background: rgba(255,255,255,0.08); padding: 5px 10px; border-radius: 12px;">
                        <button type="button" onclick="window.updateAtknQty('${itemId}', -1, '${item.name}', ${item.price})" style="background:none; border:none; color:#ff4757; font-size:20px; cursor:pointer; width:25px;">−</button>
                        <span id="qty-${itemId}" style="color: white; font-weight: bold; min-width: 20px; text-align: center;">0</span>
                        <button type="button" onclick="window.updateAtknQty('${itemId}', 1, '${item.name}', ${item.price})" style="background:none; border:none; color:#00ffcc; font-size:20px; cursor:pointer; width:25px;">+</button>
                    </div>
                </div>`;
...
window.updateAtknQty = (id, delta, name, price) => {
    ...
    if (qty > 0) selectedAtknFood[id] = { name: name, count: qty, price: price };
    else delete selectedAtknFood[id];
};

// 5. ТӨЛӨМГӨ ӨТҮҮ ЖАНА ЭСЕПТӨӨ (Step 4)
window.goToStep4 = () => {
    ...
    if (payInfo) {
        ...
        tag.innerHTML = `
            <div style="text-align:center; margin-bottom:15px; background:rgba(0,255,204,0.1); padding:15px; border-radius:15px; border: 1px solid rgba(0,255,204,0.3);">
                <h2 style="color:#00ffcc; margin:0; font-size: 24px;">${total} сом</h2>
...
window.handleAtknSubmit = async (e) => {
    ...
    try {
        ...
        window.clearAtknForm(); // Жөнөтүлгөндөн кийин тазалоо
        window.closeAtknModal(); // Модалды жабуу
