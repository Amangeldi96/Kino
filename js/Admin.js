...
function initAdminTickets() {
    ...
    onSnapshot(collection(db, "user_tickets"), (snap) => {
        ...
        snap.forEach((d) => {
            ...
            const movieName = t.movieTitle || "";
            const isAntiKino = movieName.toLowerCase().includes('антикино');

            // 1. Киши саны (count) жана жаш балдар (children)
            const peopleCount = t.count || 0; 
            const childCount = t.children || 0;

            // 2. Тамак-ашты "selectedFood" массивинен эсептөө
            let foodCount = 0;
            let drinksCount = 0;

            if (t.selectedFood && Array.isArray(t.selectedFood)) {
                t.selectedFood.forEach(item => {
                    const name = item.name.toLowerCase();
                    if (name.includes('кола') || name.includes('сок') || name.includes('суу') || name.includes('чай') || name.includes('лимонад')) {
                        drinksCount += (item.count || 0);
                    } else {
                        foodCount += (item.count || 0);
                    }
                });
            }
...
">
    ...
    <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:10px;">
        ...
        <button onclick="window.deleteTicket('${ticketId}')" style="
            background: rgba(77,77,77,0.15); border: none; color: #ffd700;
    ...
    <div style="display:flex; justify-content:space-between; align-items:flex-end; margin-bottom:12px; padding:0 2px;">
        ...
        <div style="display:flex; flex-direction:column; align-items:flex-end; gap:5px;">
            <div style="font-size:15px; font-weight:900; color:#FFD700; background:rgba(255,215,0,0.1); padding:2px 8px; border-radius:6px; border:1px solid rgba(255,215,0,0.2);">
                ${t.totalPrice || 0} <span style="font-size:10px; font-weight:400; opacity:0.8;">сом</span>
            </div>
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; background:rgba(0,0,0,0.25); padding:10px; border-radius:12px;">
        
        <div style="display:flex; gap:10px; align-items:center; flex-wrap: wrap;">
            <div style="display:flex; align-items:center; gap:3px;">
                <span class="material-icons-round" style="color:#FFD700; font-size:17px;">person</span>
                <span style="font-size:13px; font-weight:700;">${peopleCount}</span>
            </div>
            
            <div style="display:flex; align-items:center; gap:3px;">
                <span class="material-icons-round" style="color:#4ade80; font-size:17px;">child_care</span>
                <span style="font-size:13px; font-weight:700;">${childCount}</span>
            </div>

            ${isAntiKino ? `
                <div style="width:1px; height:12px; background:rgba(255,255,255,0.2); margin:0 2px;"></div>
                <div style="display:flex; align-items:center; gap:3px;">
                    <span class="material-icons-round" style="color:#ff9f43; font-size:17px;">restaurant</span>
                    <span style="font-size:13px; font-weight:700;">${foodCount}</span>
                </div>
                <div style="display:flex; align-items:center; gap:3px;">
                    <span class="material-icons-round" style="color:#0fbcf9; font-size:17px;">local_bar</span>
                    <span style="font-size:13px; font-weight:700;">${drinksCount}</span>
                </div>
            ` : ''}
        </div>
        ...
        <div style="display:flex; gap:6px;">
            ...
            <button onclick="window.viewReceipt('${receiptUrl}')" style="
                ...
                color:#FFD700; width:32px; height:32px; border-radius:8px; cursor:pointer;
                display:flex; align-items:center; justify-content:center;
            ">
